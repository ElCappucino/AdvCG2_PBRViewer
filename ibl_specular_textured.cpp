...
int main()
{
    ...
    pbrShader.use();
    pbrShader.setInt("texture_diffuse1", 0);
    pbrShader.setInt("texture_normal1", 1);
    pbrShader.setInt("texture_roughness1", 2);
    ...
    Model myModel(FileSystem::getPath("resources/objects/lion_head_4k/lion_head_4k.gltf").c_str());
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 1. Define your base static directional light vector pointing downward
        glm::vec4 baseLightDir = glm::vec4(0.0f, -1.0f, -0.5f, 0.0f);

        // 2. Build the light rotation matrix from user input
        glm::mat4 lightRotMat = glm::rotate(glm::mat4(1.0f), glm::radians(lightRotationY), glm::vec3(0.0f, 1.0f, 0.0f));

        glm::vec4 final4DDir = lightRotMat * baseLightDir;
        glm::vec3 rotatedLightDir = glm::vec3(final4DDir.x, final4DDir.y, final4DDir.z);

        glm::vec3 shadowCamPos = -rotatedLightDir * 10.0f;
        glm::mat4 lightView = glm::lookAt(shadowCamPos, glm::vec3(0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        glm::mat4 lightProjection = glm::ortho(-10.0f, 10.0f, -10.0f, 10.0f, 1.0f, 20.0f);
        glm::mat4 lightSpaceMatrix = lightProjection * lightView;
        glm::mat4 envMatrix = glm::rotate(glm::mat4(1.0f), glm::radians(envRotationY), glm::vec3(0.0f, 1.0f, 0.0f));

        // 1. Activate the PBR Shader and set global scene matrices
        pbrShader.use();
        
        pbrShader.setMat4("envRotationMatrix", envMatrix);
        pbrShader.setVec3("lightDirection", rotatedLightDir);
        pbrShader.setMat4("lightSpaceMatrix", lightSpaceMatrix);
        glm::mat4 view = camera.GetViewMatrix();
        pbrShader.setMat4("view", view);
        pbrShader.setVec3("camPos", camera.Position);

        // 2. Bind the pre-computed IBL maps (Keep this! It's what makes the PBR pop)
        ...
        // 3. Render your custom model instead of the primitive spheres
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.0f)); // Center it
        model = glm::scale(model, glm::vec3(2.0f, 2.0f, 2.0f));     // Adjust scale if it's too big/small
        ...
        backgroundShader.use();
        backgroundShader.setMat4("envRotationMatrix", envMatrix);
